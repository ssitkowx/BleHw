...
void BleSerializer::Turn (cJSON * v_root)
{
    add (v_root, "Turn", static_cast<double>(Settings::GetInstance ().Wehicle.eTurn));
}

void BleSerializer::Drive (cJSON * v_root)
{
    add (v_root, "Drive", static_cast<double>(Settings::GetInstance ().Wehicle.eDrive));
    add (v_root, "Speed", static_cast<double>(Settings::GetInstance ().Wehicle.Speed));
}
